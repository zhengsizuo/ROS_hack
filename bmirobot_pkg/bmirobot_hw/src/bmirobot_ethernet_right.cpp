...
void CtrmsgCallback(const bmirobot_msg::Robot_ctr::ConstPtr& msg)
{
	...
	//ROS_ERROR("right ctr recieve: [%d]",msg->mtID[7]);
...
main(int argc, char **argv)
{
  ...
  ros::Publisher Mt_fdctrpub = n.advertise<bmirobot_msg::Robot_fdctr>("rMT_fdctr", 100);
  ros::Publisher Mt_fdstatuspub = n.advertise<bmirobot_msg::Robot_fdstatus>("rMT_fdstatus", 100);
  ros::Publisher Mt_mpupub = n.advertise<bmirobot_msg::Robot_mpu>("rMT_fdmpu", 1000);
  ros::Subscriber MT_ctrsub = n.subscribe("rMT_ctr", 1000, CtrmsgCallback);
  ...
  string deviceip;
  n.getParam("/right_IP", deviceip);

  int remoteport;
  n.getParam("/right_port", remoteport);
...
//deviceip1=(char *)deviceip;char* aaa = append.c_str()
  ...
  //servaddr2.sin_addr.s_addr =  inet_addr(DSET_IP_ADDRESS);
	...
	path = getcwd(NULL, 0);
	if(saveflag=="true")
		sprintf(tmp,"/home/bmi/bmiproject/ws_pc/data/%d-%d-%d-%d-%d-%d-rightmtfd.csv", 1900+info->tm_year, 1+info->tm_mon, info->tm_mday,info->tm_hour,info->tm_min,info->tm_sec);
	else
		sprintf(tmp,"/home/bmi/bmiproject/ws_pc/data/new-rightmtfd.csv");
	std::cout<<tmp<<"sdf"<<endl;
	mtfdout.open(tmp, std::ofstream::out | std::ofstream::binary);

	if(saveflag=="true")
		sprintf(tmp,"/home/bmi/bmiproject/ws_pc/data/%d-%d-%d-%d-%d-%d-rightmpufd.csv", 1900+info->tm_year, 1+info->tm_mon, info->tm_mday,info->tm_hour,info->tm_min,info->tm_sec);	
	else
		sprintf(tmp,"/home/bmi/bmiproject/ws_pc/data/new-rightmpufd.csv");
	std::cout<<tmp<<endl;
  ...
  while (ros::ok())
  {
	...
	int rnum=sendto(sock1, sendcmd, 64, 0, (struct sockaddr *)&servaddr1, sizeof(servaddr1));
	if(rnum == -1 )
	{
		perror("send right fail.\n");
	}else
	{
		//ROS_INFO("send right num:%d",rnum);
	}

	for(int i=0;i<2;i++)
	{
	    ret = recvfrom(sock1, recvbuf, sizeof(recvbuf), 0, (struct sockaddr *)&servaddr1, &socklen);
	    //ROS_INFO("right sucessful:%d",ret);
		if (ret == 373)
	    {
	        recvbuf[ret]='\0';
	        for(int i=0;i<8;i++)
			...
			ROS_INFO("right :%d,%d,%d",ret,fdctrmsg.mt_mode[0],fdctrmsg.mt_Cpst[0]);
			for(int i=0;i<8;i++)
			{
