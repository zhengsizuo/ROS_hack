^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package bmirobot_control_cart
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* velocity_controllers::BmirobotControlCart: New plugin variant for
...
0.8.1 (2014-07-11)
------------------
* bmirobot_control_cart: Critical targets declared before calling catkin_package
...
0.5.3 (2013-09-04)
------------------
* bmirobot_control_cart: New package implementing a controller for executing joint-space trajectories on a
  set of joints.

  * ROS API

    * Commands: FollowJointTrajectory action and trajectory_msgs::JointTrajectory topic.
    * Current controller state is available in a control_msgs::BmirobotControlCartState topic.
    * Controller state at any future time can be queried through a control_msgs::BmirobotControlCartState
      service.
